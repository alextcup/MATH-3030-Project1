...
A \textit{Euclidean domain} is a mathematical structure (a \textit{ring}, more specifically an \textit{integral domain}) in which a division algorithm holds. In other words, a Euclidean domain is one where the ideas of quotients, remainders and congruence makes sense. %The canonical example of a Euclidean domain is $\mathbb{Z}$, of which other Euclidean domains can be considered generalizations.
...
where $d$ is a squarefree integer % ADD A REFERENCE!!!
